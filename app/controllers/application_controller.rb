class ApplicationController < ActionController::Base
  ...
  def configure_permitted_parameters
    devise_parameter_sanitizer.for(:sign_up) { |u| u.permit(:email, :password, :password_confirmation, :name, :country, :language_id, :fname, :gender, :city, :occupation_id, :organization_name, :organization_url, :user_source_id, :birthday, {sector_ids: []}, {topic_ids: []}, :terms_of_service)}
    devise_parameter_sanitizer.for(:account_update) << [:fname, :name, :email, :country, :language_id, :occupation_id, :organization_url, :organization_name, :gender, :city, :email_address, :password, :password_confirmation, :bio, :facebook_url, :twitter_url, :google_plus_url, :github_url, :linkedin_url, :profile_type, :tag_list, :image, :title, {sector_ids: []}, {topic_ids: []}]
  end
